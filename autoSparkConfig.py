...
cfg_base_array = {
  ...
  'counter_table' :                 'false',
  'max_counter_update_index' :      '0',
  'ttl' :                           'false',
  'ttl_index' :                     '0',
  'timestamp_filter' :              'false',
  'timestamp_index' :               '0',
  'min_timestamp_filter' :          '0',
  'max_timestamp_filter' :          '0',
  'random_partitioner' :            'false',
  'min_partition' :                 '-9223372036854775808',
  'max_partition' :                 '9223372036854775807'
...
def process_field_value(fieldType):
  if 'map<' in fieldType:
    mapData = fieldType.split('<')[1].split('>')[0].split(',')
    fValue = field_type_array['map']
    for mapType in mapData:
      fValue += '%' + field_type_array[mapType.strip()]
      field_type_comment(tbl,fieldName,mapType.strip())
  elif 'set<' in fieldType:
    mapData = fieldType.split('<')[1].split('>')[0].split(',')
    fValue = field_type_array['set']
    for mapType in mapData:
      fieldValue += '%' + field_type_array[mapType.strip()]
      field_type_comment(tbl,fieldName,mapType.strip())
  elif fieldType in field_types:
    fValue = field_type_array[fieldType]
  else:
    exit('Error: unknown field type: ' + fieldType +'\n'+cql)
  return str(fValue)
  
def process_field(tbl,fieldName,fieldType,cql='',primary=0):
  fieldValue = process_field_value(fieldType)
  cfg_array['fields'] += fieldName
  cfg_array['field_types'] += fieldValue
  if primary==1:
    cfg_array['primary_field_types'] += ','+fieldValue
    cfg_array['primary_field_types'] = cfg_array['primary_field_types'].lstrip(',')
    cfg_array['insert_field_types'] += ','+fieldValue
  else:
    cfg_array['non_primary_field_types'] += fieldValue+','
  if fieldType == 'counter':
    cfg_array['counter_update_cql'] = cfg_array['counter_update_cql'].replace('<<counter_field>>',fieldName)
...
if len(target_field_type)==0 and len(target_table)==0:
  target_field_type.append('all')
...
for line in schemaFile:
  ...
  if (line==''): tbl=''
    ...
    if('CREATE INDEX' in line):
      ...
      tbl_data[ks]['table'][tbl]['special_fields'] = []
      ...
      tbl_line = line.split()[2].strip('"')
      tbl = tbl_line.split('.')[1].strip().strip('"')
      tbl_data[ks]['table'][tbl] = {'type':'Table', 'cql':''}
      tbl_data[ks]['table'][tbl]['field'] = {}
      tbl_data[ks]['table'][tbl]['special_fields'] = []
      ...
      if('FROM' in line and tbl_data[ks][tbl]['type']=='Materialized Views'):
        ...
        try:
          ...
          if('AND ' not in line and ' WITH ' not in line):
            ...
            if fld_type == 'counter' : tbl_data[ks]['table'][tbl]['special_fields'].append('counter')
            if (fld_name!='CREATE'):
              tbl_data[ks]['table'][tbl]['field'][fld_name]=fld_type
        except:
  ...
  if (migrate_ks == '' or migrate_ks == ks):
    for tbl, tblData in list(ksData['table'].items()):
      if tblData['type'] == 'Table':
        if len(target_table)>0 or 'all' in target_field_type:
          if tbl in target_table or 'all' in target_field_type:
...
for tbl,tblData in list(migrate_tbl_data.items()):
  ...
  for line in defaultsFile:
    line = line.strip('\n').strip()
    if len(line.split()) > 1:
      cfg_array[line.split()[0]] = line.split()[1].strip()
  ...
  cfg_array['insert_field_types'] = ''
  cfg_array['insert_fields'] = ''
  cfg_array['primary_field_types'] = ''
  cfg_array['counter_update_index'] = ''
  cfg_array['counter_update_cql'] = ''
  cfg_array['non_primary_field_types'] = ''
  first_field = 1

  if 'counter' in tblData['special_fields']:
    cfg_array['counter_update_cql'] = 'update '+cfg_array['keyspace_table']+' set <<counter_field>> +=? where'
    counter_x = 0
  for key in tblData['pk']:
    if first_field == 1:
      first_field = 0
      cfg_array['partition_keys'] = key
      cfg_array['primary_keys'] = key
      if 'counter' in tblData['special_fields'] :
        cfg_array['counter_update_cql'] += ' '+key+'=?'
        cfg_array['counter_update_index'] += str(counter_x)
        counter_x += 1
    else:
      cfg_array['fields'] += ','
      cfg_array['field_types'] += ','
      ...
      cfg_array['primary_keys'] += key
      if 'counter' in tblData['special_fields'] :
        cfg_array['counter_update_cql'] += ' and '+key+'=?'
        cfg_array['counter_update_index'] += ','+str(counter_x)
        counter_x += 1
    process_field(tbl,key,tblData['field'][key],tblData['cql'],1)
    cfg_array['fields'] += ','
  ...
  if 'counter' in tblData['special_fields'] :
    ...
    cfg_array['fields'] += ',writetime('+fieldName+'),ttl('+fieldName+')'
    cfg_array['counter_update_index'] = str(counter_x)+','+cfg_array['counter_update_index']
    cfg_array['timestamp_index'] = str(counter_x+1)
    cfg_array['ttl_index'] = str(counter_x+2)
    cfg_array['insert_field_types'] = cfg_array['non_primary_field_types'].strip(',') + cfg_array['insert_field_types']
  else:
    cfg_array['counter_table'] = 'false'
    cfg_array['timestamp_index'] = '0'
    cfg_array['counter_update_index'] = '0'
    cfg_array['ttl_index'] = '0'
    cfg_array['insert_fields'] = cfg_array['fields']
    cfg_array['insert_field_types'] = cfg_array['field_types']

  if cfg_array['random_partitioner'] == 'true':
    cfg_array['min_partition']= '-1'
    cfg_array['max_partition']= '170141183460469231731687303715884105728'
