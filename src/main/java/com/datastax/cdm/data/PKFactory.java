...
public class PKFactory {
    ...
    public EnhancedPK getTargetPK(Row originRow) {
        ...
        if (logDebug) logger.debug("getTargetPK: newValues: {}; {}; explodeMapTargetKeyColumnIndex={}", newValues, null==writetimeTTLFeature?WritetimeTTL.class.getSimpleName()+":null":writetimeTTLFeature, explodeMapTargetKeyColumnIndex);
        ...
        if (explodeMapTargetKeyColumnIndex < 0) {
            ...
            return new EnhancedPK(this, newValues, getPKClasses(Side.TARGET), originTTL, originWriteTimeStamp, explodeMap);
        }
    }
    ...
    public BoundStatement bindWhereClause(Side side, EnhancedPK pk, BoundStatement boundStatement, int startingBindIndex) {
        ...
        for (int i=0; i<indexesToBind.size(); i++) {
            ...
            boundStatement = boundStatement.set(startingBindIndex++, pk.getPKValues().get(index), table.getPKClasses().get(index));
        }
    ...
    private List<Object> getTargetPKValuesFromOriginColumnLookupMethod(Row originRow, List<Object> defaultValues) {
        ...
        for (int i = 0; i< targetPKLookupMethods.size(); i++) {
            ...
            int originIndex = targetTable.getCorrespondingIndex(targetTable.indexOf(targetTable.getPKNames(false).get(i)));
