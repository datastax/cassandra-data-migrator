...
public class TIMESTAMP_StringFormatCodec extends AbstractBaseCodec<String> {
    ...
    public TIMESTAMP_StringFormatCodec(PropertyHelper propertyHelper, CqlHelper cqlHelper) {
        super(propertyHelper, cqlHelper);

        if (cqlHelper.isCodecRegistered(Codecset.TIMESTAMP_STRING_MILLIS))
            throw new RuntimeException("Codec " + Codecset.TIMESTAMP_STRING_MILLIS + " is already registered");
