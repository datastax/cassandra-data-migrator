...
RUN mkdir -p /assets/ && cd /assets && \
    curl -fsSL https://api.github.com/repos/datastax/dsbulk/releases/latest | grep -o 'https://github.com/datastax/dsbulk/releases/download/[^"]*\.tar\.gz' | head -n 1 | xargs curl -L -o dsbulk.tar.gz && \
